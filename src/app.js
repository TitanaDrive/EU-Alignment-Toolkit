...
function updateCoupling(time) {
  const couplingRadius = 76;
  const preGateSync = smoothstep(0.25, 0.95, nodes.filter((n) => n.x < gate.x).length / nodes.length);
  ...
  for (let i = 0; i < nodes.length; i += 1) {
    ...
    for (let j = 0; j < nodes.length; j += 1) {
      ...
      const dx = a.x - b.x;
      const dy = a.y - b.y;
      if (dx * dx + dy * dy > couplingRadius * couplingRadius) continue;
    ...
    if (!a.aligned && a.x < gate.x + gate.halfWidth) {
      ...
      a.phase += toGlobal * (0.004 + preGateSync * 0.01);
    }
...
function updateNode(node, i, w, h, time, alignedRatio) {
  ...
  const proximity = smoothstep(gate.x - w * 0.35, gate.x + w * 0.02, node.x);

  node.vx += dx * field.centerAttraction;
  node.vy += dy * field.centerAttraction;

  node.vy += node.pulse * params.drift * 0.6;
  node.vx += Math.cos(time * 0.0013 + node.phase) * params.drift * 0.12;

  const funnelStrength = smoothstep(gate.x - w * 0.26, gate.x - w * 0.02, node.x);
  const funnelCenterY = gate.y + node.pulse * 8;
  node.vy += (funnelCenterY - node.y) * (0.012 + funnelStrength * 0.024);
  ...
  node.vx += params.gatePull + gateApproach * field.gateBoost + preCrossBoost * (0.06 * params.gateSpike);
  node.vx += proximity * 0.02;
